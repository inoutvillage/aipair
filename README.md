...
| 必要 | 最小 | 検証済みバージョン（2026-09-12 実測） | 備考 |
|---|---|---|---|
...
| `claude` | — | 2.1.268 | Claude Code CLI（`npm install -g @anthropic-ai/claude-code`、要ログイン） |
| `codex` | — | 0.154.0 | OpenAI Codex CLI（`npm install -g @openai/codex`、要ログイン） |
...
| 🔄 relay 再点火（非endless・版無視） | `aipair-relay-here --no-endless --allow-untested-dialogs`（**起動済みペア**へ**非 endless**（単発・stop ワードで終了）で張り直す。プラン/質問リレーを版差でも継続。endless が要らない単発レビュー向け） |
...
2. Codex の返答で分岐:
   ...
   - **承認**（先頭行に `[AIPAIR_PLAN_APPROVED]` を単独で）→ 先頭の「Yes…」（claude 2.1.247・2.1.268 では「Yes, and use auto mode」。旧 UI の「Yes, and bypass permissions」が出る版ではそちらを優先）を選択して実装開始
...
Claude Code / Codex CLI の版が変わると壊れうる。そこで relay は**ペインで実際に動いている CLI の版**を判定する
（npm の自動更新はディスク上のファイルを差し替えるだけで、動いている TUI は起動時の版のまま。PATH の `--version` では
読んでいない版を判定してしまう）。

- **Linux / WSL**: ペインの CLI プロセスを `/proc` から特定し、その実体（`/proc/<pid>/exe`。更新で削除済みでも可）に
  `--version` を問う。起動時に判定するほか、**Claude のダイアログに触れる直前に毎回ペインの claude を再判定**する
  （検知後にペインで再起動された場合も捕まえる）。プロセスを特定できない・版を確かめられない時は **PATH を信用せず操作しない**。
- **`/proc` の無い環境（macOS 等）**: 実行中の版は分からないため、従来どおり PATH の `claude --version` / `codex --version`。
- 起動ログには版の出どころ（ペインで実行中 / PATH）と、ディスク上の版が違う時はその版も表示する。

**検証済み版（上の「必要環境」表）と一致しない、または確かめられない**場合は、

- **プラン承認・質問リレーの自動操作だけを OFF**（起動ログ／実行ログに理由を表示）
- **poke による往復・transcript の読み取りは通常どおり継続**（＝ペアの相互レビューは動く）
- ただし Claude がダイアログで止まっている時に、レビューを**通常の poke でダイアログへ打ち込むことはしない**
  （数字が選択肢として解釈されうるため）。画面に触れず exit 5 で停止する
...
`bin/aipairlib/__version__`（**リリース間の main は `X.Y.Z-dev.N`**＝開発版。main のチェックアウトがリリース版を名乗らないようにするため。suffix を外すのはリリースコミットだけ）、リリースは git タグ `v<version>` とその GitHub Release（`v*` タグの push で
