...
`aipair-relay` は**同じディレクトリ**の `peer-log`・`aipair-corelib`（純粋ヘルパ）・`aipair-loglib`（トランスクリプト読取）・`aipair-tmuxlib`（tmux 実行/ペイン操作）・`aipair-deliverylib`（poke 配達/Enter 送信）を読み込む（`SourceFileLoader`）。
