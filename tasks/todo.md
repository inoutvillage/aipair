...
- [x] **P2-1 relay state machine を state 単位で分割**（state_machine/review_protocol/schema_guard/plan_flow/question_flow）。relay.py は arg parse/依存構築/起動/exit に寄せる。
  ...
  - **P2-1 完了（`- [x]`）**: named module（state_machine / review_protocol / schema_guard / plan_flow / question_flow）が全て出揃い、状態機械の**テスト可能な判定核**（ResponseGate＝応答帰属・decide_plan_action＝プラン承認・decide_question_action＝質問中継）を単体テスト化。relay.py は分割開始時 ~1470 → **894 行**（−576）。**ループ骨格（state 遷移本体）は意図的に relay の orchestration 核へ残置** — 640 行の密結合 `nonlocal` ループで単体テストが本質的に効かない統合レベルのコード、丸ごと移設は本番ペア駆動経路でリスク最大のため、グローバル規則17（本番自動化に実機で確証できない推測改良を入れない）＋ A6 の既決に従う。fail-closed 不変条件は全増分で不変更。
