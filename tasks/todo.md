...
- [x] **P2-2 installer の transaction 性強化**（package + entrypoints + templates を staging→検証→一括切替、Phase2 途中失敗は entrypoint も自動 rollback）。
  - 完了: **package + entrypoints を単一 STAGE のバッチトランザクション化**。旧 Phase2 は entrypoint を `install_one` で逐次コピーし、途中失敗で「新 package/新 aipair の隣に旧 aipair-relay」という mixed 状態が残った。新実装は package と entrypoints を同一 `.aipair-stage-$TS` へ staging → **staged thin entrypoint を staged package に対して `--help` 実行**（entrypoint は自ディレクトリを sys.path に足すので、co-located stage が新しい組合せそのものを検証）→ 変更分のみ一括 swap（`SWITCHED` 配列で追跡）。**mv/backup 失敗・swap 後 --help 失敗のいずれでも `_rollback_install` が package も全 entrypoint も復元**（.bak-$TS から）＝all-or-nothing。templates/通知ブロック（install_block）は別ロケーションの独立ファイルのため per-file で既に staging（temp→`os.replace`）→ディスク検証→失敗時 backup 復元済み（適切な粒度）。skills も per-file backup。テスト `install-upgrade.sh` に **Phase2 途中失敗の全 entrypoint ロールバック**検証を追加（peer を dir 化して 4番目の switch を失敗させ、先行3つが pre-upgrade 内容へ復元＋stage 掃除を確認、42 checks）。run-all 10 系統緑。
