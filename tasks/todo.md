...
- [ ] #8/#9 CI を実 CLI の nightly smoke/E2E ＋ Python 3.8/tmux 3.1 の matrix に。（実装は完了、残は認証付き E2E の**実走検証**のみ）
  ...
  - [ ] **認証付き round-trip E2E**（実装は完了、`[x]`＝検証済みの定義に照らし**実走検証は保留** — Codex P2 relay-id:7f778749）— `nightly.yml` の E2E step を、両 secret がある時に一時 dir で実ペアを起動→固有 nonce プロンプトを claude に注入→**full round-trip（下記3シグナル）を ~240s** タイムアウトで確認し、成否を問わず session を撤去（trap）する検証へ置換。fail-closed（1つでも欠ければ FAIL＋bridge/転写ダンプ）。tmux サーバは起動プロセスの env を継承するため smoke が残したサーバを `kill-server`→鍵入りで fresh 起動、`aipair loop` の detached セッションを `script`+`timeout` で起こして pane へ注入。`AIPAIR_STOP_SIDE=claude` を明示（codex の「完了です」で return hop が省略される偽失敗を防ぐ）。
    ...
    - 実走試行（Codex 指示 relay-id:91631a4b）: repo の Actions secrets は **0 件**（`ANTHROPIC_API_KEY`/`OPENAI_API_KEY` 未設定・API で名前確認）。`workflow_dispatch` を実行（https://github.com/inoutvillage/aipair/actions/runs/32602627712）→ nightly は success、**E2E step は secret 無しで self-skip**（`skip: ... needs BOTH ... — not run.` を出力し exit 0）を実 CI で確認。3シグナルの認証パスは未実行のため**チェックせず**（Codex 条件「secret 不足時はチェックしない」に従う）。
