...
- [ ] #8/#9 CI を実 CLI の nightly smoke/E2E ＋ Python 3.8/tmux 3.1 の matrix に。（実装は完了、残は認証付き E2E の**実走検証**のみ）
  ...
  - [ ] **認証付き round-trip E2E**（実装は完了、`[x]`＝検証済みの定義に照らし**実走検証は保留** — Codex P2 relay-id:7f778749）— 実装は Codex 指示 relay-id:ebcd91b6 で完了: `nightly.yml` の E2E step を、両 secret がある時に一時 dir で実ペアを起動→固有 nonce プロンプトを claude に注入→**claude 転写に nonce・codex rollout に `relay-id:`（relay の配達 nonce）の両方**をタイムアウト付き（~180s）で確認し、成否を問わず session を撤去（trap）する round-trip 検証へ置換。fail-closed（片方でも欠ければ FAIL＋bridge/転写ダンプ）。tmux サーバは起動プロセスの env を継承するため smoke が残したサーバを `kill-server`→鍵入りで fresh 起動、`aipair loop` の detached セッションを `script`+`timeout` で起こして pane へ注入。
    - 判定は fail-closed で、**claude が assistant ターンで nonce を返した**こと（`<SID>.jsonl` を直読し `type==assistant` 本文のみ照合／注入 user プロンプトの nonce では成立しない＝Codex P1 対応）と、**codex rollout に `relay-id:`** が届いたことの両方を要求。
    - 検証（secret 無しで可能な範囲）: YAML パース・`bash -n`・skip/one-secret パス（両方 exit 0）を実走。**nonce 検出 python を合成 JSONL 4 ケースで単体テスト**（assistant 一致=真／user のみ=偽／string content=真／不在=偽、全 OK）。検出マーカー `relay-id:[0-9a-f]+` は稼働中ループの実 codex rollout（93 件）で確証（`deliverylib.py:85` の stamp と一致）。
