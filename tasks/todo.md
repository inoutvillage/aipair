...
- [ ] **P2-1 relay state machine を state 単位で分割**（state_machine/review_protocol/schema_guard/plan_flow/question_flow）。relay.py は arg parse/依存構築/起動/exit に寄せる。
  ...
  - **現状（受入条件 未充足・`- [ ]` に差し戻し・Codex レビュー relay-id:3298e7a2 指摘）**: 判定核（ResponseGate＝応答帰属・decide_plan_action＝プラン承認・decide_question_action＝質問中継）は単体テスト化したが、**受入条件「relay.py を arg parse／依存構築／起動／exit に寄せる」が未達** — `while True` の全 state 遷移が relay に残り、`plan_flow.py`／`question_flow.py` も未作成。判定関数の抽出だけでは「named module が出揃った」とは言えない、との指摘は妥当。当初 A6＋規則17 でループ骨格を残置と判断したが、受入条件（＋レビュー）が明示的に分割を要求するため方針転換する。
  - **残作業（P2-1 完了の条件）**: `while True` の状態機械本体を `state_machine.py` の `StateMachine` クラス（loop 状態を属性化・`wait_heartbeat`/`claude_watch` 等の closure をメソッド化・`run()→exit code`）へ移し、relay.main() を「parse → 依存構築 → `StateMachine(...).run()` → exit」の**ランチャのみ**に縮約する。挙動保存はフルスイート＋稼働ペアへの self-host で検証（ループ本体は統合レベルで単体テストが効かないため、self-host が最終確証）。これは最もリスクの高い変更のため専念ターンで慎重に実施する。
