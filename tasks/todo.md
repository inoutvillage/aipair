...
- [x] #8/#9 CI を実 CLI の nightly smoke/E2E ＋ Python 3.8/tmux 3.1 の matrix に。
  ...
  - [x] **認証付き round-trip E2E を実装**（Codex 指示 relay-id:ebcd91b6）— `nightly.yml` の E2E step を、両 secret がある時に一時 dir で実ペアを起動→固有 nonce プロンプトを claude に注入→**claude 転写に nonce・codex rollout に `relay-id:`（relay の配達 nonce）の両方**をタイムアウト付き（~180s）で確認し、成否を問わず session を撤去（trap）する round-trip 検証へ置換。fail-closed（片方でも欠ければ FAIL＋bridge/転写ダンプ）。tmux サーバは起動プロセスの env を継承するため smoke が残したサーバを `kill-server`→鍵入りで fresh 起動、`aipair loop` の detached セッションを `script`+`timeout` で起こして pane へ注入。
    - 検証: YAML パース・`bash -n`・skip/one-secret パス（両方 exit 0）を実走で確認。**検出マーカー `relay-id:[0-9a-f]+` を稼働中ループの実 codex rollout（93 件一致）で確証**（`deliverylib.py:85` の stamp と一致）。認証パス自体の実走は secret＋クリーン runner が要る（既定サーバに触れられない実機では起動不可＝設計上 CI 専用）。
    - 残（外部依存のみ）: 実行には ANTHROPIC/OPENAI の repo secret＋毎 nightly の API 予算が要る（社長/管理者権限）。secret が揃えば step は自動で round-trip を実走し、揃わなければ skip。
