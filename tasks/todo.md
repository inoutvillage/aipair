...
- [ ] #8/#9 CI を実 CLI の nightly smoke/E2E ＋ Python 3.8/tmux 3.1 の matrix に。
  ...
  - [ ] 残: **認証付き round-trip E2E**（実 poke→応答→relay）— ANTHROPIC/OPENAI の repo secret ＋ API 予算が要る。nightly.yml に self-skip する step の scaffold は配置済み。secret 設定は社長/管理者権限が必要（外部依存）。
