...
- [ ] **P2-4 versioned release 運用開始**（`aipair --version` / git tag / CHANGELOG.md / GitHub Releases）。
  - **リリース基盤の準備完了（`- [ ]` のまま。実際の v0.1.0 tag/Release が未発行のため未完了。ローカル/origin ともタグ無し）。実際の発行は outward-facing のため社長判断・私は push しない＝tag push まで release workflow は dormant。Codex レビュー relay-id:bd4265fa 反映で完了扱いを取消**: **単一の source of truth `bin/aipairlib/__version__`="0.1.0"** を定義し、`aipair --version`（bash→co-located package を python で読む）・`aipair-relay --version`（argparse `action=version`）が同値を出力。**CHANGELOG.md**（Keep a Changelog・`## [0.1.0] - 2026-08-23`＋`[Unreleased]`）、**RELEASING.md**（版 bump→CHANGELOG→PR→CI→tag `v<version>` push→Release の手順）、**`.github/workflows/release.yml`**（`v*` tag push で tag==`__version__` を検証→該当 CHANGELOG 節を notes に抽出→`gh release create`）。**doc-sync に `Version`（5 tests）追加**: `__version__` が SemVer／CHANGELOG 最新リリース版==`__version__`／`aipair`・`aipair-relay` の `--version` 出力==`__version__`／release.yml が tag を `__version__` に固定。README に「版・リリース」節。run-all 11 系統・doc-sync 20 tests 緑。TESTED_VERSIONS（検証済み CLI 版）とは別物。
