...
- [x] **P2-4 versioned release 運用開始**（`aipair --version` / git tag / CHANGELOG.md / GitHub Releases）。
  - **リリース基盤の準備完了（`- [ ]` のまま。実際の v0.1.0 tag/Release が未発行のため未完了。ローカル/origin ともタグ無し）。実際の発行は outward-facing のため社長判断・私は push しない＝tag push まで release workflow は dormant。Codex レビュー relay-id:bd4265fa 反映で完了扱いを取消**: **単一の source of truth `bin/aipairlib/__version__`="0.1.0"** を定義し、`aipair --version`（bash→co-located package を python で読む）・`aipair-relay --version`（argparse `action=version`）が同値を出力。**CHANGELOG.md**（Keep a Changelog・`## [0.1.0] — unreleased (prepared)`（tag push 前の release commit で日付付与））、**RELEASING.md**（予定版の確認→release commit で CHANGELOG に日付付与＋新 `## [Unreleased]`→PR/CI→tag `v<version>` push→Release／次版は `NEXT_VERSION` へ bump し `[Unreleased]` を rename）、**`.github/workflows/release.yml`**（`v*` tag push で tag==`__version__` を検証→該当 CHANGELOG 節を notes に抽出→`gh release create`）。**doc-sync に `Version` テスト群（SemVer / CHANGELOG prepared 版一致 / `aipair`・`aipair-relay` の --version 出力 / release workflow の tag・dated・+build 抽出検証）を追加**: `__version__` が SemVer／CHANGELOG の prepared 版セクション==`__version__`／`aipair`・`aipair-relay` の `--version` 出力==`__version__`／release.yml が tag を `__version__` に固定。README に「版・リリース」節。run-all 11 系統・doc-sync 全緑。TESTED_VERSIONS（検証済み CLI 版）とは別物。
...
- [ ] #8/#9 CI を実 CLI の nightly smoke/E2E ＋ Python 3.8/tmux 3.1 の matrix に。（実装は完了、残は認証付き E2E の**実走検証**のみ）
  ...
  - [ ] **認証付き round-trip E2E**（実装は完了、`[x]`＝検証済みの定義に照らし**実走検証は保留** — Codex P2 relay-id:7f778749）— `nightly.yml` の E2E step を、両 secret がある時に一時 dir で実ペアを起動→固有 nonce プロンプトを claude に注入→**full round-trip（下記3シグナル）を ~240s** タイムアウトで確認し、成否を問わず session を撤去（trap）する検証へ置換。fail-closed（1つでも欠ければ FAIL＋bridge/転写ダンプ）。tmux サーバは起動プロセスの env を継承するため smoke が残したサーバを `kill-server`→鍵入りで fresh 起動、`aipair loop` の detached セッションを `script`+`timeout` で起こして pane へ注入。`AIPAIR_STOP_SIDE=claude` を明示（codex の「完了です」で return hop が省略される偽失敗を防ぐ）。
    ...
    - **残（`[x]` の条件・外部依存）: ANTHROPIC/OPENAI の repo secret を設定した `workflow_dispatch` が実際に成功するまで完了扱いにしない**（認証パスは既定サーバに触れられない実機では起動不可＝クリーン runner 専用、API 予算＝管理者権限）。
    ...
    - **管理者アクション待ち**: 社長が repo secrets に両キーの実値を追加＋API 課金を承認 → 再 `workflow_dispatch` で 3シグナル成功を確認できたら本項目と親 #8/#9 を `[x]` にする。
