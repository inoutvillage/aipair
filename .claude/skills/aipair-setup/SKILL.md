...
▼ 自走ループの既定値
  停止ワード [AIPAIR_REVIEW_OK]（AIPAIR_STOP）／ 判定側 codex（AIPAIR_STOP_SIDE）／ 最大 20 往復（AIPAIR_MAX_ROUNDS）
  連続モードの合図 [AIPAIR_NEXT]／終端 2 種: 全完了 [AIPAIR_ALL_DONE]・人間対応待ち [AIPAIR_HUMAN_REQUIRED]（AIPAIR_NEXT_ASK / AIPAIR_ALL_DONE / AIPAIR_HUMAN_REQUIRED）
