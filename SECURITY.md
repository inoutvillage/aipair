...
- **境界／緩和:**
  ...
  - 連続モードで**人間対応・外部依存でしか進められないタスクは `tasks/todo.md` に `- [!]`（直下に `blocker:` 理由）で表現**し、着手可 `- [ ]` が尽きて `- [!]` だけ残ると relay は無限往復せず **HUMAN_REQUIRED として停止（exit 8）**して人間の対応を待ちます（誤った自律継続を避ける fail-closed 挙動）。
